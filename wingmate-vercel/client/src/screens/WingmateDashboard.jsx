...
import { getPusherClient, subscribeToUser } from '../lib/pusher';
...
export default function WingmateDashboard() {
  ...
  // Listen for incoming session requests via Pusher user channel
  useEffect(() => {
    const client = getPusherClient();
    if (!client || !user?.id) return;
    const channel = client.channel(`private-user-${user.id}`);
    if (!channel) return;
    channel.bind('wingmate:request', (data) => setIncoming(data));
    return () => channel.unbind('wingmate:request');
  }, [user?.id]);
  ...
  function declineRequest() {
    if (incomingRequest) {
      ...
      body: JSON.stringify({ sessionId: incomingRequest.sessionId }),
    }).catch(console.error);
    }
    setIncoming(null);
  }
